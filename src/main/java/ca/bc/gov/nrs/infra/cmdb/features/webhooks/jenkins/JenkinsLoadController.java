...
@Api(value = "/load/jenkins", description = "Webhook endpoints for loading data from Jenkins", consumes = "application/json")
@RestController
...
//    }
    ...
    @ApiOperation(
            value = "Attempts to create a batch of build records, useful for loading historical data. Allows updates to existing builds.",
            response = UpsertBuilds.Model.class
    )
    @PostMapping("/build")
    public UpsertBuilds.Model addBuilds(@RequestBody UpsertBuilds.Command message,
                          HttpServletResponse response)
    {
        return this.mediator.send(message, UpsertBuilds.Model.class);
    }
